...
![Figure 1: @product@'s file system store creates a folder structure based on primary keys in the database.](../../images/enterprise-file-system-store.png)
...
## Migrating to a Different Store on an Existing Installation

If you've already been uploading files to @product@ and you want to move to a
different store implementation, it is not enough to change the `dl.store.impl`
property in `portal-ext.properties`. Instead, you must migrate your files from
the old store to the new one. Here's how: 

1. Open the *Menu* (![Menu](../../images/icon-menu.png)) and navigate to
   *Control Panel &rarr; Configuration &rarr; Server Administration &rarr; Data
   Migration*.

2. Use the `dl.store.impl` selector to choose the desired storage type.
...
5. Change the `dl.store.impl` property in the `portal-ext.properties` to the
   storage type you chose at step 2.

6. Start the portal application or the servlet container.

Your files are now migrated to the new store, and any new files are also
uploaded there. 
