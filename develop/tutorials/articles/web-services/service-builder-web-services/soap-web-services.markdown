...
invocation. For details, please see the
[Web Service Security Layers (not yet written)]() article.

$$$

As an example, let's look at some example SOAP web service clients for
Liferay's `Company`, `User`, and `UserGroup` portal services that perform these
tasks:

1. List each user group to which the user with the screenname *test* belongs.
...
service interfaces. If you haven't used Service Builder before, please see the
[Introduction to Service Builder (not yet written)]() tutorial.
...
okay. The SOAP web service invocations worked!
...
using SOAP web services. To learn how to implement OAuth so that you can access
third-party services, please see the
[Authorizing Access to Services with OAuth (not yet written)]() tutorial.
