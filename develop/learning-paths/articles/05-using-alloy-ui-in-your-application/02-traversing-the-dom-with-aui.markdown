...
4. add the html element just above the `<aui:form>` opening tag we just made:
    ...
    For now a empty `<div>` has been created with a unique id. We'll append an 
    HTML element that will display our silly phrase later on. Now that we have 
    the foundation layer, we can add the logic to make it work.
...
1. Add this aui sandbox just below the `aui-char-counter` script we created in 
...
7. Add this code below the message variable:
    ...
    We use the `html()` method to set the inner HTML of the message element and 
    string together.
