...
2.  Create a new subclass with old the name (e.g. `DeployUtil extends MyDeployUtil`)
...
After executing the target, you should see the logs of several operations that copy files from the extension environment and into the equivalent directory within the Ext plugin (see the section *Creating an Ext plugin* for an explanation of the main directories within the plugin).
